...
Web 设置面板中的分区（固定在设置页侧栏最顶部）：三张卡按序展示——OpenCode Zen 免费档、Go 档、DeepSeek 官方段（llm-deepseek）；每行带勾选框，底部「删除选中」批量操作可跨三张卡任意组合一次确认删减，下一次请求即从对话模型列表移除——已配置条目（下架的带标记）、线上未配置 id 的勾选列表、行内移除、两击同步（先预览再确认）。读写完全走既有配置页协议——`settings.describe` / 带 `expectedRevision` 的 `settings.update`，实时列表走 `llm.discoverModels`——仅在页面打开时跟随推送失效刷新。
