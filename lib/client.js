        ...
        return Promise.all([
          ...
          setState(function (current) {
            ...
            for (var i = 0; i < TIERS.length; i++) {
              ...
              // Paid ids ride the free listing too; the free card only ever
              // sees the "-free" family plus ids this user already configured.
              if (tier.id === 'free') {
                var keepIds = configured.map(function (entry) { return entry.id; });
                liveResult = liveResult.filter(function (entry) {
                  return entry.id.endsWith('-free') || keepIds.indexOf(entry.id) >= 0;
                });
              }
