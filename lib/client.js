...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var ZH = {
      ...
      others: '其他',
      otherRoute: 'llm-deepseek · DeepSeek 官方',
      otherHint: '官方段（llm-deepseek）无线上列表，仅展示已配置条目；批量删除后即从对话模型列表移除，下一次请求生效。',
      otherNone: '（未配置官方模型）',
      selectedCount: '已选模型：',
      deleteSelected: '删除选中',
      confirmDelete: '确认删除',
    ...
    var EN = {
      ...
      others: 'Other',
      otherRoute: 'llm-deepseek · DeepSeek official',
      otherHint: 'The official section (llm-deepseek) has no live listing; it shows configured entries only. Bulk removal drops them from the conversation model list on the next request.',
      otherNone: '(no official models configured)',
    ...
    /** Third card: the official DeepSeek section (llm-deepseek), no live listing. */
    function OtherCard(props) {
      var t = props.t;
      var state = props.state.other || {};
      ...
      var writable = state.writable !== false;

      function removeId(id) {
        if (busy || !writable) return;
        setBusy(true);
        setMessage(null);
        props.withFreshDescriptor(OTHER_NS).then(function (descriptor) {
          var models = descriptor && descriptor.value && Array.isArray(descriptor.value.models)
            ? descriptor.value.models.filter(function (entry) { return entry.id !== id; })
            : [];
          return unwrap(
            props.api.settings.update({ ns: OTHER_NS, patch: { models: models }, expectedRevision: descriptor.revision }),
            'settings.update'
          );
        }).then(function () {
          setBusy(false);
      ...
      return React.createElement('section', { className: 'ocm-tier' },
        React.createElement('div', { className: 'ocm-tier-head' },
          React.createElement('span', { className: 'ocm-tier-title' }, t('others')),
          React.createElement('span', { className: 'ocm-tier-route' }, t('otherRoute')),
          React.createElement('span', { className: 'ocm-count' }, t('configured') + ' ' + configured.length)
        ),
        state.error !== undefined
          ...
          : React.createElement('div', null,
              ...
              configured.length === 0
                ? React.createElement('div', { className: 'ocm-hint' }, t('otherNone'))
                : configuredRows(t, {
                    entries: configured,
        ...
        React.createElement('div', { className: 'ocm-hint' }, t('otherHint')),
        message ? React.createElement('div', { className: message.kind === 'error' ? 'ocm-error' : 'ocm-ok' }, message.text) : null
      );
    }
    ...
    function OpenCodeSection(props) {
      ...
      var load = React.useCallback(function () {
        ...
        var dsDescriptorPromise = describedPromise.then(function (value) {
          return value.namespaces.filter(function (ns) { return ns.ns === OTHER_NS; })[0];
        });
        return Promise.all([
          descriptorPromise,
          dsDescriptorPromise,
          ...
          var dsDescriptor = results[1];
          setState(function (current) {
            var next = { status: 'ready', writable: results[0].writable !== false, free: {}, go: {}, other: {}, selected: {}, bulkConfirm: false, notice: current.notice };
            for (var i = 0; i < TIERS.length; i++) {
              var tier = TIERS[i];
              var liveResult = results[i + 2];
            ...
            if (dsDescriptor && dsDescriptor.value && Array.isArray(dsDescriptor.value.models)) {
              next.other = { configured: dsDescriptor.value.models.slice(), writable: dsDescriptor.writable !== false };
            } else {
              next.other = { configured: [], writable: dsDescriptor ? dsDescriptor.writable !== false : true };
            }
            return next;
          });
        }).catch(function (error) {
      ...
      function executeBulkDelete() {
        ...
        function writeAt(index) {
          ...
          var ns = item === 'other' ? OTHER_NS : SETTINGS_NS;
          return Promise.resolve(describeSettings(props.api)).then(function (value) {
            var descriptor = value.namespaces.filter(function (n) { return n.ns === ns; })[0];
            if (!descriptor) throw new Error(ns + ': namespace missing');
            if (descriptor.writable === false) throw new Error(ns + ': ' + t('readOnly'));
            if (item !== 'other') {
              ...
              var models = profile && Array.isArray(profile.models)
                ...
                : [];
              return unwrap(
                props.api.settings.update({ ns: ns, patch: buildPatch(TIERS_MAP[item], models), expectedRevision: descriptor.revision }),
                'settings.update'
              );
            }
            var otherModels = descriptor.value && Array.isArray(descriptor.value.models)
              ? descriptor.value.models.filter(function (entry) { return ids.indexOf(entry.id) < 0; })
              : [];
            return unwrap(
              props.api.settings.update({ ns: ns, patch: { models: otherModels }, expectedRevision: descriptor.revision }),
              'settings.update'
            );
          }).then(function () {
