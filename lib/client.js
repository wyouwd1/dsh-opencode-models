...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function describeNamespace(api) {
      return unwrap(api.settings.describe({}), 'settings.describe').namespaces.filter(function (ns) {
        return ns.ns === SETTINGS_NS;
      })[0];
    }
    ...
    function TierCard(props) {
      ...
      function sync() {
        ...
        setConfirming(false);
        if (addedIds.length === 0 && staleIds.length === 0 || busy) return;
        run(function () {
          return props.withFreshDescriptor().then(function (descriptor) {
      ...
      return React.createElement('section', { className: 'ocm-tier' },
        ...
        React.createElement('div', { className: 'ocm-actions' },
          selectedCount > 0
            ? React.createElement(Button, { type: 'button', variant: 'primary', size: 'sm', disabled: busy, onClick: addSelected },
                t('addSelected') + ' (' + selectedCount + ')')
            : null,
          !state.error && (addedIds.length > 0 || staleIds.length > 0)
            ? React.createElement(Button, {
                type: 'button', variant: 'outline', size: 'sm', disabled: busy, onClick: sync
    ...
    function OpenCodeSection(props) {
      ...
      var load = React.useCallback(function () {
        ...
        var descriptorPromise = Promise.resolve(describeNamespace(props.api));
        return Promise.all([
          descriptorPromise,
      ...
      var withFreshDescriptor = React.useCallback(function () {
        return Promise.resolve(describeNamespace(props.api));
      }, []);
