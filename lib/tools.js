 ...
 */
async function liveList(llm, tier, signal, configuredIds) {
  ...
  // Paid ids ride the free listing too; never let them count, drift or adopt.
  return tier.id === 'free' ? filterFreeTierLive(list, configuredIds) : list
}
...
async function routeReport(resolve, tierId, signal) {
  ...
  try {
    ...
    const live = await liveList(llm, tier, signal, configured.models.map((entry) => entry.id))
    ...
    async execute(args, exec) {
      ...
      for (const tierId of tierIds) {
        ...
        try {
          ...
          const live = await liveList(llm, tier, exec.signal, current.models.map((entry) => entry.id))
          const liveById = new Map(live.map((entry) => [entry.id, entry]))
